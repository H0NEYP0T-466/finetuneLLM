...
### Text-Based History Storage
- All conversations are saved to `history.txt`
- Persistent chat history across server restarts
- Simple text format for easy debugging and inspection
...
- `GET /messages?limit=20` - Get last N messages from history file
...
From the root directory, use docker-compose to run the backend:
...
- ✅ Mounts the model directory
- ✅ Persists history.txt file

### Manual Docker Deployment

When running in Docker manually:
1. Ensure the model file is mounted to `/app/model/`
2. Ensure history.txt is mounted or the directory is writable
3. CPU usage will spike to 100% during generation (this is normal)
4. RAM usage depends on model size (typically 4-8GB for 7B models)
